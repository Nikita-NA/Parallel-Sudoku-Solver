...
int main(int argc, char** argv)
{
    ...
    if (solver->get_status()) {
        ...
        std::cout << termcolor::yellow << "No solution found with the selected mode." << termcolor::reset << "\n";
        // Return non-zero so callers can detect failure
        #if PRINT_TIME
        stop = std::chrono::high_resolution_clock::now();
        int time_in_microseconds = std::chrono::duration_cast<std::chrono::microseconds>(stop - start).count();
        std::cout << std::dec << "[Stopped in " << (double) time_in_microseconds / 1e6 << " seconds.]" << "\n";
        #endif
        return 2;
    }
